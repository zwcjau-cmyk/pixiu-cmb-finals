...
@app.delete("/api/user/finance-data")
...
async def script_chat_endpoint(req: ScriptChatRequest):
    ...
    def event_stream():
        ...
        if not image_url and full_text and len(full_text.strip()) > 20:
            ...
            # 直接用已生成的剧情回复作为画面语义，避免额外的 JSON 提取步骤失败导致丢图。
            result_json = GenerateComicPlotTool().generate_story_comic(
                full_text[:400],
                "剧情进展",
            )
